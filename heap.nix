...
let
  percDown =
    ...
    if (idx * 2 + 1) > heap.size then
      ...
      if !hasRhs then
        if cmpLhs < 0 then
          percDown (idx * 2) ({
            inherit (heap) cmp size;
            data = swap heap.data idx (idx * 2);
          })
        else
          heap
      # if we're less than lhs || rhs we swap with the lesser of the two
        ...
        percDown lessIdx {
          inherit (heap) cmp size;
          data = swap heap.data idx lessIdx;
        }
      # in the right spot
      else
        heap;
  ...
  percUp =
    ...
    if idx == 0 then
      ...
      in
      if (heap.cmp parentVal val) > 0 then
        percUp parentIdx ({
          inherit (heap) cmp size;
          data = swap heap.data idx parentIdx;
        })
      else
        heap;
in
      ...
      let
        ...
        heap' = {
          ...
          data = builtins.genList (
            i: if i == 0 then elemAt heap.data (heap.size - 1) else elemAt heap.data i
          ) (heap.size - 1);
        };
        # Reheapify
