    ...
    let
      ...
      fixLine = line:
        let
          ...
          firstBad =
            findFirst (pair: p.badSeq ? "${pair}") null (lineCombos line);
        in if firstBad == null then
          line
          ...
          let
            ...
            swapped =
              swap line ((findFirst (el: el.char == (head badEl)) null iline).i)
              ((findFirst (el: el.char == (elemAt badEl 1)) null iline).i);
          in fixLine swapped;
    in (foldl'
      (acc: el: acc + (toInt (builtins.elemAt (fixLine el) ((length el) / 2))))
      0 p.lines) - p1Answer;
